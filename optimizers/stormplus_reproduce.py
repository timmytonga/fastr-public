...
class STORMplus(Optimizer):
    def __init__(self, params, lr=required, weight_decay=0., init_accumulator=1., c=1., normalized_norm=False):
        assert c > 0 or c == -1
        assert init_accumulator >= 0 or init_accumulator == -1
        ...
        assert c >= 0 and init_accumulator >= 0
        self.normalized = normalized_norm
        self.t = 0
        self.c = c
        self.G_t = 0.
        self.D_t = 0.
    ...
    def compute_estimator(self, closure=None):
        normalized_norm = self.normalized
        loss = None
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                state['d_t'] = (state['current_grad'] + (1. - self.a_t) * (
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if weight_decay != 0:
                    p_grad.add_(weight_decay, p.data)
                state = self.state[p]
                p.data.add_(other=state['d_t'], alpha=-eta_t)
        return loss
    ...
    def param_c(self):
        ...
        return num_params**(2/3)

    def param_init_acc(self):
        num_params = self.get_num_params_of()
        return num_params
